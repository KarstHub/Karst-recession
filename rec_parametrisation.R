...
xv <- rec.extract(x,  model = "Vogel", plot = T)
xb <- rec.extract(x, model= "Brut", plot= T)
# #ganez recession
# yy <- rbind(x[[60]][2]$Q)
...
rec_coeff_para <- function(x, plot=TRUE, method=c("linear", "lower_env", "RMS"))
    ...
    x <- rbindlist(xv) %>%   
        ...
        mutate("dQdt_matrix2" = (lead(Q_Matrix_rec, n=1) - lag(Q_Matrix_rec, n = 1))/2) # dQ/dt: Ableitung der matrix recession über zwei 2 zeitschritte

    if (method=="lower_env"){
        rec_mod <- rq(log10(-dQdt_matrix2) ~ log10(Q_Matrix_rec), data = x, tau = 0.05)
        
        rec_cons <- 1/10^(rec_mod$coefficients[1])
        ...
        if (plot== TRUE){
            ...
            plot(
                log10(-x$dQdt_matrix2) ~ log10(x$Q_Matrix_rec),
                xlab = "log10(Q)",
                ylab = "log10(-dQ/dt )",
                ylim = range(log10(-x$dQdt_matrix2),na.rm=T),
                xlim = range(log10(x$Q_Matrix_rec),na.rm=T),
                main= "")
    ...
    if (method=="linear"){
        ...
        rec_mod <- lm(log10(- x$dQdt_matrix2) ~log10(x$Q_Matrix_rec))
...
# rec_coff_para(xv, method="linear", plot=T)
# rec_coff_para(xb, method="lower_env", plot=T)
